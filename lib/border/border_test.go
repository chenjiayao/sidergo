...
func TestParserBorder(t *testing.T) {
	...
	if !(border1.Include && border1.Inf == 1 && border1.Value == 12.3) {
		t.Errorf("parase %s failed", s1)
	}

	s2 := "(12.3"
	border2, _ := ParserBorder(s2)
	if !(!border2.Include && border2.Inf == 1 && border2.Value == 12.3) {
	...
	if !(!border4.Include && border4.Inf == -1 && border4.Value == 0) {
